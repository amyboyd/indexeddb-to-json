...
const {timestampForFilename, handleUncaughtExceptionsAndRejections} = require('./utils');
...
const initialJsToEvaluateOnPage = async () => {
    ...
    const jsToEvaluateOnPageForEachStore = async (db, connection, storeName, dbExportObject) => {
...
(async () => {
    ...
    const chromeIndexedDbDir =
        chromeDir + '/Default/IndexedDB/' + host.replace(/^(https?):\/\//, '$1_') + '_0.indexeddb.leveldb';
    ...
    const sourceDatabasesDir = source.replace('IndexedDB', 'databases').replace('.indexeddb.leveldb', '');
